...
public abstract class ModelGenTestCase {
	...
	protected Comparator<VHDLSpecification> vhdlComp;
	protected MODELGENStopCriterion stop;
	protected boolean flatten;
	
	protected ModelGenTestCase(String flatten) {
		this.flatten = flatten.equals("eMoflon::Ibex Flattened") ? true : false;
	}
	...
	public void createGenerator() throws IOException {
		generator = new MODELGEN_App("VHDLTGGCodeAdapter", "./../", flatten, true);
		...
		vhdlComp = new VHDLModelComparator(false);
	}
...
//		System.out.println((System.nanoTime() - startTime)/1000000.0);
	...
	protected void assertPostconditionOnTrg(String trg) {
		...
		for (int i = 0; i < trgExp.getContents().size(); i++) {
			vhdlComp.assertEquals((VHDLSpecification)trgExp.getContents().get(i), 
					                  (VHDLSpecification)generator.getTargetResource().getContents().get(i));
		}
	}
	

	@Parameters(name = "{0}")
	public static Collection<String> flattening() throws IOException {
		return Arrays.asList("eMoflon::Ibex Flattened", "eMoflon::Ibex Refinement");
	}
}
