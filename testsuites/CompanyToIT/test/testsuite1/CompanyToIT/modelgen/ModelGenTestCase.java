...
public abstract class ModelGenTestCase {
	...
	protected Comparator<Company> companyComp;
	protected Comparator<IT> itComp;
	...
	public void createGenerator() throws IOException {
		generator = new MODELGEN_App("CompanyToIT", "./../", flatten, false);
		...
		companyComp = new CompanyLanguageComparator(false);
		itComp = new ITLanguageComparator(false);
	}
...
//		System.out.println((System.nanoTime() - startTime)/1000000.0);
	...
	protected void assertPostconditionOnSrc(String src) {
		...
		for (int i = 0; i < srcExp.getContents().size(); i++) {
			companyComp.assertEquals((Company)srcExp.getContents().get(i), 
					                  (Company)generator.getSourceResource().getContents().get(i));
		}
	}
	...
	protected void assertPostconditionOnTrg(String trg) {
		...
		for (int i = 0; i < trgExp.getContents().size(); i++) {
			itComp.assertEquals((IT)trgExp.getContents().get(i), 
					                  (IT)generator.getTargetResource().getContents().get(i));
		}
	}
