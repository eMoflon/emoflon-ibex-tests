...
public class PerformanceTestSYNC extends PerformanceTest<SYNC> {
	...
	public List<TestDataPoint> timedExecutionAndInit(Supplier<SYNC> transformator, int size, int repetitions,
			boolean isFwd, Consumer<EObject> edit) throws IOException {
		if (repetitions < 1)
		...
		for (int i = 0; i < repetitions; i++) {
			...
			System.out.println((isFwd ? Operationalization.FWD : Operationalization.BWD) + ": size=" + size + ": "
					+ (i + 1) + "-th execution started.");
			...
			if (useTimeouts)
				try {
					...
					batchExecutionTimes[i] = batchExecutionResult
							.get(useTimeouts ? PerformanceConstants.timeout : Long.MAX_VALUE, TimeUnit.SECONDS);
					...
					incrementalExecutionTimes[i] = incrExecutionResult
							.get(useTimeouts ? PerformanceConstants.timeout : Long.MAX_VALUE, TimeUnit.SECONDS);
		...
		batchData.testCase = new TestCaseParameters(tgg.getName(),
				isFwd ? Operationalization.FWD : Operationalization.BWD, size);

		TestDataPoint incData = new TestDataPoint(initTimes, incrementalExecutionTimes);
		incData.testCase = new TestCaseParameters(tgg.getName(),
				isFwd ? Operationalization.INCREMENTAL_FWD : Operationalization.INCREMENTAL_BWD, size);
