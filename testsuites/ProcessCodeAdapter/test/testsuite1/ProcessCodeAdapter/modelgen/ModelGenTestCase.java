...
public abstract class ModelGenTestCase {
	...
	protected Comparator<SystemModule> systemComp;
	protected Comparator<Folder> mocaTreeComp;
	...
	public void createGenerator() throws IOException {
		...
		systemComp = new ProcessDefinitionComparator(false);
		mocaTreeComp = new MocaTreeFolderComparator(false);
	}
...
//		System.out.println((System.nanoTime() - startTime)/1000000.0);
	...
	protected void assertPostconditionOnSrc(String src) {
		...
		for (int i = 0; i < srcExp.getContents().size(); i++) {
			mocaTreeComp.assertEquals((Folder)srcExp.getContents().get(i), 
					                  (Folder)generator.getSourceResource().getContents().get(i));
		}
	}
	...
	protected void assertPostconditionOnTrg(String trg) {
		...
		for (int i = 0; i < trgExp.getContents().size(); i++) {
			systemComp.assertEquals((SystemModule)trgExp.getContents().get(i), 
					                  (SystemModule)generator.getTargetResource().getContents().get(i));
		}
	}
