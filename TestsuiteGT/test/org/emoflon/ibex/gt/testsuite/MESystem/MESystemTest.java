...
public class MESystemTest extends MESystemAbstractTest {
	...
	/*
	 * @Test //Two NAC + NAC in refined rule Test(max 5 cables on core and max 1
	 * different cable type) public void checkDenseCablesOnCoreNACAllLocal() {
	 * GtGtApi<?> api = this.initEmpty("checkDenseCablesOnCoreNACAllLocal.xmi");
	 * 
	 * assertMatchCount(1, api.mesystem());
	 * assertApplicableAndApply(api.mesystem());
	 * assertNotApplicable(api.mesystem()); assertApplicableAndApply(api.core());
	 * 
	 ...
	 * assertApplicableAndApply(api.denseCable());
	 * 
	 * //find 5 densecables assertMatchCount(5, api.findCableOnCore());
	 * 
	 * //6 cables should not possible assertNotApplicable(api.denseCable());
	 * 
	 * terminate(api); }
	 * 
	 * @Test //Two NAC + NAC in refined rule Test(max 5 cables on core and max 1
	 * different cable type) public void checkSmartCablesOnCoreNACAllLocal() {
	 ...
	 * assertApplicableAndApply(api.smartCable());
	 * 
	 * //find 5 smartcables assertMatchCount(5, api.findCableOnCore());
	 * 
	 * //6 cables should be not possible assertNotApplicable(api.smartCable());
	 * 
	 * terminate(api); }
	 */

	@Test
	// Two NAC + NAC in refined rule Test(max 5 cables on core and max 1 different
	// cable type)
	...
	// Tests as the one aboth for 2 different interfaces but disabled due to top
	// class constraint not checked
	/*
	 * @Test //Three NAC + NAC in refined rule Test(max 5 terminals on cable and max
	 * 1 different interface type) public void checkTerminalOnCableWithLocal() {
	 ...
	 * assertApplicableAndApply(api.denseCable());
	 * 
	 * for(int i = 0; i < 6; i++) { assertApplicableAndApply(api.terminal()); }
	 * 
	 * //find 5 terminals assertMatchCount(6, api.findTerminalOnCable());
	 * 
	 * //only 5 terminals per cable assertNotApplicable(api.terminal());
	 * 
	 * terminate(api); }
	 * 
	 * @Test //Three NAC + NAC in refined rule Test(max 5 meinterfaces on cable and
	 * max 1 different interface type) public void checkInterfaceOnCableWithLocal()
	 ...
	 * assertApplicableAndApply(api.denseCable());
	 * 
	 * for(int i = 0; i < 25; i++) { assertApplicableAndApply(api.meinterface()); }
	 * 
	 * //find 25 interfaces assertMatchCount(25, api.findMEInterfaceOnCable());
	 * 
	 * //max 5 interfaces per cable assertNotApplicable(api.meinterface());
	 * 
	 * terminate(api); }
	 */

	@Test
	// Three NAC + NAC in refined rule Test(max 5 terminals on cable and max 1
	...
	/*
	 * @Test //Three NAC + NAC in refined rule Test(max 5 meinterfaces on cable and
	 * max 1 different interface type) public void
	 * checkInterfaceOnCableExclusiveWithLocal2() { GtGtApi<?> api =
	 * this.initEmpty("checkInterfaceOnCableExclusiveWithLocal2.xmi");
	 * 
	 * assertMatchCount(1, api.mesystem());
	 * assertApplicableAndApply(api.mesystem());
	 * assertNotApplicable(api.mesystem()); assertApplicableAndApply(api.core());
	 ...
	 * assertApplicableAndApply(api.denseCable());
	 * 
	 * for(int i = 0; i < 4; i++) { assertApplicableAndApply(api.meinterface()); }
	 * 
	 * for(int i = 0; i < 5; i++) { assertApplicableAndApply(api.terminal()); }
	 * 
	 * //find 25 interfaces assertMatchCount(4, api.findMEInterfaceOnCable());
	 * 
	 * //max 5 interfaces per cable assertNotApplicable(api.terminal());
	 * 
	 * terminate(api); }
	 */
	...
	@Test
	// 5 NAC + 2 NAC in refined rule Test(max 5 storagecells per drive and only 1 of
	// each type)
	...
	/*
	 * @Test //6 NAC + 2 NAC in refined rule Test(max 10 items per storagecells)
	 * public void checkItemsInStorageWithLocal() { GtGtApi<?> api =
	 * this.initEmpty("checkItemsInStorageWithLocal.xmi");
	 * 
	 * assertMatchCount(1, api.mesystem());
	 * assertApplicableAndApply(api.mesystem());
	 * assertNotApplicable(api.mesystem()); assertApplicableAndApply(api.core());
	 ...
	 * 
	 * //find 10 items assertMatchCount(10, api.find1ItemInStorage());
	 * 
	 * //only 1 Type of each per drive assertNotApplicable(api.itemInStorage());
	 * 
	 * terminate(api); }
	 * 
	 * @Test //6 NAC + 2 NAC in refined rule Test(max 10 items per storagecells)
	 * public void checkStorageCellInDriveExclusiveWithLocal2() { GtGtApi<?> api =
	 * this.initEmpty("checkStorageCellInDriveExclusiveWithLocal2.xmi");
	 * 
	 * assertMatchCount(1, api.mesystem());
	 * assertApplicableAndApply(api.mesystem());
	 * assertNotApplicable(api.mesystem()); assertApplicableAndApply(api.core());
	 ...
	 * assertApplicableAndApply(api.denseCable());
	 * 
	 * for(int i = 0; i < 10; i++) { assertApplicableAndApply(api.bus()); }
	 * 
	 * for(int i = 0; i < 10; i++) { assertApplicableAndApply(api.drive()); }
	 * 
	 * for(int i = 0; i < 10; i++) { assertApplicableAndApply(api.storageCell16K());
	 * }
	 * 
	 * for(int i = 0; i < 10; i++) { assertApplicableAndApply(api.storageCell1K());
	 * }
	 * 
	 * for(int i = 0; i < 10; i++) { assertApplicableAndApply(api.storageCell4K());
	 * }
	 * 
	 * for(int i = 0; i < 10; i++) { assertApplicableAndApply(api.storageCell64K());
	 * }
	 * 
	 * //find 10 storageCells of each Type assertMatchCount(10,
	 * api.find1KStorageCell()); assertMatchCount(10, api.find16KStorageCell());
	 * assertMatchCount(10, api.find4KStorageCell()); assertMatchCount(10,
	 * api.find64KStorageCell());
	 * 
	 * //only 1 Type of each per drive assertNotApplicable(api.storageCell1K());
	 * assertNotApplicable(api.storageCell64K());
	 * assertNotApplicable(api.storageCell16K());
	 * assertNotApplicable(api.storageCell4K());
	 * 
	...
	@Test
	// 6 NAC + 2 PAC + 1 Variable constraint Test(max 9 items, no recipe with same
	// name, max 5 recipes per terminal, enforce that there is at least one drive
	// and 1 storage cell)
	 ...
	 * GtGtApi<?> api =
	 * this.initEmpty("checkItemsInStorageCellConstrainExclusiveWithLocal2.xmi");
	 * 
	 * assertMatchCount(1, api.mesystem());
	 * assertApplicableAndApply(api.mesystem());
	 * assertNotApplicable(api.mesystem()); assertApplicableAndApply(api.core());
	 ...
	 * 
	 * for(int i = 0; i < 10; i++) { assertApplicableAndApply(api.storageCell64K());
	 * } for(int i = 0; i < 100; i++) { assertApplicableAndApply(api.item("wood"));
	 * }
	 * 
	 * //find 10 storageCells of each Type assertMatchCount(10,
	 * api.find1KStorageCell()); assertMatchCount(10, api.find16KStorageCell());
