...
public class FamilyNormaliser implements Comparator<Family>{
	...
	public int compare(Family family1, Family family2) {
		...
		{
			int f_sibling = createStringFromSiblings(family1.getSiblings()).compareTo(createStringFromSiblings(family2.getSiblings()));
			if(f_sibling!=0)
			{
				return f_sibling;
			}
	...
	private String createStringFromSiblings(List<Family> members)
	{
		Comparator<Family> comparator = new FamilySiblingNormaliser();
		ArrayList<Family> membersToBeSorted = new ArrayList<>(members);
		Collections.sort(membersToBeSorted, comparator);
		return membersToBeSorted.stream().map(Family::getName).collect(Collectors.joining("-"));
	}
