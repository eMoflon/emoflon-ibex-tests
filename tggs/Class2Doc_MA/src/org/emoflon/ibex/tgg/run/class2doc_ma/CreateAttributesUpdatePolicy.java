...
/**
 * This update policy prefers adding attribute first to super class.
 * 
 * If super class already have an attribute, then all other attributes are added
 * to subclass.
 */
public class CreateAttributesUpdatePolicy extends UpdatePolicy {
	...
	protected Optional<IMatch> forceClassCreation(ImmutableMatchContainer matchContainer) {
		Optional<IMatch> match = matchContainer.getMatches().stream()
				.filter(m -> m.getPatternName().equals("Class2Doc__GEN")).findAny();
		return match;
	}

	protected Optional<IMatch> forceInheritanceCreation(ImmutableMatchContainer matchContainer) {
		Optional<IMatch> match = matchContainer.getMatches().stream()
				.filter(m -> m.getPatternName().equals("SubClass2SubDoc__GEN")).findAny();
		return match;
	}
}
